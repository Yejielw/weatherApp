...
var pushComment = function () {
 ...
 citiesWeather[$id].comments.push(text)
 $('div[data-comments="' + $id + '"]').empty();
  for (var i = 0; i < citiesWeather[$id].comments.length; i++) {
   ...
   $('div[data-comments="' + $id + '"]').append(newHTML);
...
$('.cityWeather').on('click','.post-comment', function () {
  //$comment = $(this).closest(".input").val()
  text = $(this).siblings('.comment-input').val();
  //$comment= $(this).find()'.comment-input'.val();// $(this).find('.comment-input').val();
  $id = this.dataset.id
  console.log($id)
